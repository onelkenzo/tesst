...
-- ========================================
-- Hacks Panel (NEW - moved from Combat to fix overflow)
-- ========================================
local HacksPanel = Window:CreatePanel({
    Name = "Hacks",
    DisplayName = "Netrunner Hacks"
})

-- Spam Hack Logic
getgenv().SpamHack = false
getgenv().HackTargetMode = "All Players" -- "All Players", "Specific Player"
getgenv().SelectedHack = "SHORT-CIRCUIT" -- Default
getgenv().TargetPlayerName = "" -- For specific targeting

local hackNames = {"PLACE-MARKER", "SELF-SABOTAGE", "CREDIT-LIFT", "BLINDSHOT", "SHORT-CIRCUIT"}

UILib:CreateDropdown(HacksPanel, {
    Label = "Select Hack to Spam",
    Options = hackNames,
    Callback = function(val)
        getgenv().SelectedHack = val
    end
})

UILib:CreateDropdown(HacksPanel, {
    Label = "Target Mode",
    Options = {"All Players", "Specific Player"},
    Callback = function(val)
        getgenv().HackTargetMode = val
    end
})

UILib:CreateButton(HacksPanel, {
    Text = "Set Target Player Name",
    Callback = function()
        -- Get list of all players for easy selection
        local playerNames = {}
        ...
        if #playerNames > 0 then
            -- Show first available player as example
            getgenv().TargetPlayerName = playerNames[1]
            UILib:CreateNotification({Text = "Target set to: " .. playerNames[1] .. " (Click again to cycle)", Duration = 3})
            
            -- Store index for cycling
            if not getgenv().TargetPlayerIndex then
                getgenv().TargetPlayerIndex = 1
            else
                getgenv().TargetPlayerIndex = getgenv().TargetPlayerIndex + 1
                if getgenv().TargetPlayerIndex > #playerNames then
                    getgenv().TargetPlayerIndex = 1
                end
                getgenv().TargetPlayerName = playerNames[getgenv().TargetPlayerIndex]
                UILib:CreateNotification({Text = "Target set to: " .. getgenv().TargetPlayerName, Duration = 3})
            end
        else
            UILib:CreateNotification({Text = "No other players found!", Duration = 3, Color = UILib.Colors.ERROR})
        end
    end
})

-- Click to Target Feature
getgenv().ClickToTargetEnabled = false

UILib:CreateToggle(HacksPanel, {
    Label = "Enable Click to Target",
    Default = false,
    Callback = function(state)
        getgenv().ClickToTargetEnabled = state
        
        if state then
            UILib:CreateNotification({Text = "Click on a player to target them!", Duration = 3})
            
            -- Create mouse click listener
            if not getgenv().ClickTargetConnection then
                local UserInputService = game:GetService("UserInputService")
                local Mouse = LocalPlayer:GetMouse()
                
                getgenv().ClickTargetConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
                    if not getgenv().ClickToTargetEnabled then return end
                    if gameProcessed then return end
                    
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        -- Raycast from mouse to find what was clicked
                        ...
                        if result and result.Instance then
                            ...
                            local clickedChar = result.Instance:FindFirstAncestorOfClass("Model")
                            
                            if clickedChar and clickedChar:FindFirstChild("Humanoid") then
                                -- Find the player who owns this character
                                for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
                                    if player.Character == clickedChar and player ~= LocalPlayer then
                                        getgenv().TargetPlayerName = player.Name
                                        getgenv().HackTargetMode = "Specific Player"
                                        UILib:CreateNotification({Text = "Target locked: " .. player.Name, Duration = 3, Color = UILib.Colors.JPUFF_HOT_PINK})
                                        return
                                    end
                                end
                            end
            ...
            UILib:CreateNotification({Text = "Click to Target disabled", Duration = 2})
            if getgenv().ClickTargetConnection then
                getgenv().ClickTargetConnection:Disconnect()
                getgenv().ClickTargetConnection = nil
            end
        end
    end
})

-- Cooldown Remover Toggle
getgenv().RemoveCooldowns = false

UILib:CreateToggle(HacksPanel, {
    Label = "Remove Hack Cooldowns",
    Default = false,
    Callback = function(state)
        getgenv().RemoveCooldowns = state
        
        if state then
            task.spawn(function()
                while getgenv().RemoveCooldowns do
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Values") then
                        for _, val in ipairs(LocalPlayer.Character.Values:GetChildren()) do
                            local name = val.Name
                            -- Remove ANY cooldown value
                            if string.find(name, "CD") or string.find(name, "Cooldown") or string.find(name, "cooldown") then
                                val:Destroy()
                            end
                        end
                    end
            ...
            UILib:CreateNotification({Text = "Cooldown Remover Enabled!", Duration = 2})
        else
            UILib:CreateNotification({Text = "Cooldown Remover Disabled", Duration = 2})
        end
    end
})

-- Hack Rotation Toggle
getgenv().RotateHacks = false
getgenv().HackRotationIndex = 1

UILib:CreateToggle(HacksPanel, {
    Label = "Auto Rotate Hacks",
    Default = false,
    Callback = function(state)
        getgenv().RotateHacks = state
        
        if state then
            task.spawn(function()
                local hacksList = {"PLACE-MARKER", "SELF-SABOTAGE", "CREDIT-LIFT", "BLINDSHOT", "SHORT-CIRCUIT"}
                
                while getgenv().RotateHacks do
                    local MyChar = LocalPlayer.Character
                    local MyRoot = MyChar and MyChar:FindFirstChild("HumanoidRootPart")
                    
                    -- Set stamina to 100
                    if MyChar and MyChar:FindFirstChild("Values") and MyChar.Values:FindFirstChild("Stamina") then
                        MyChar.Values.Stamina.Value = 100
                    end
                    
                    if MyRoot then
                        -- Get current hack in rotation
                        local currentHack = hacksList[getgenv().HackRotationIndex]
                        ...
                        if combatRemote then
                            ...
                            if getgenv().HackTargetMode == "Specific Player" then
                                local targetName = getgenv().TargetPlayerName or ""
                                if targetName ~= "" then
                            ...
                            -- Fire current hack on targets
                            for _, targetChar in ipairs(targets) do
                                local hum = targetChar:FindFirstChild("Humanoid")
                                local root = targetChar:FindFirstChild("HumanoidRootPart")
                                
                                if hum and root and hum.Health > 0 then
                                    combatRemote:FireServer(currentHack, targetChar)
                                end
                            end
                            
                            -- Rotate to next hack
                            getgenv().HackRotationIndex = getgenv().HackRotationIndex + 1
                            if getgenv().HackRotationIndex > #hacksList then
                                getgenv().HackRotationIndex = 1
                            end
                        end
                    end
                    
                    task.wait(0.5) -- Fire different hack every 0.5 seconds
                end
            end)
            UILib:CreateNotification({Text = "Hack Rotation Enabled!", Duration = 2})
        else
            UILib:CreateNotification({Text = "Hack Rotation Disabled", Duration = 2})
        end
    end
})

UILib:CreateButton(HacksPanel, {
    Text = "Execute Hack on Target",
    Callback = function()
        local MyChar = LocalPlayer.Character
        local MyRoot = MyChar and MyChar:FindFirstChild("HumanoidRootPart")
        
        -- Set stamina to 100 (server checks >= 40)
        if MyChar and MyChar:FindFirstChild("Values") and MyChar.Values:FindFirstChild("Stamina") then
            MyChar.Values.Stamina.Value = 100
        end
        
        if MyRoot then
            local hackName = getgenv().SelectedHack or "SHORT-CIRCUIT"
            local remotes = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes")
            local combatRemote = remotes and remotes:FindFirstChild("Combat") and remotes.Combat:FindFirstChild("NetrunnerHack")
            ...
            if combatRemote then
                ...
                if getgenv().HackTargetMode == "Specific Player" then
                    -- Target specific player by name
                    local targetName = getgenv().TargetPlayerName or ""
                    if targetName ~= "" then
                        local targetPlayer = game:GetService("Players"):FindFirstChild(targetName)
                        if targetPlayer and targetPlayer.Character then
                        ...
                        UILib:CreateNotification({Text = "No target selected! Use 'Set Target Player' button.", Duration = 3, Color = UILib.Colors.ERROR})
                        return
                    end
                else
                    -- Target ALL players
                ...
                for _, targetChar in ipairs(targets) do
                    ...
                    if hum and root and hum.Health > 0 then
                        combatRemote:FireServer(hackName, targetChar)
                        hackedCount = hackedCount + 1
                    end
                end
                
                if hackedCount > 0 then
                    UILib:CreateNotification({Text = "Executed " .. hackName .. " on " .. hackedCount .. " target(s)", Duration = 2})
                else
                    UILib:CreateNotification({Text = "No valid targets found!", Duration = 2, Color = UILib.Colors.ERROR})
                end
            else
                UILib:CreateNotification({Text = "Combat remote not found!", Duration = 2, Color = UILib.Colors.ERROR})
            end
        else

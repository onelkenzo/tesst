...
local function PerformAutoSell()
    ...
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character.HumanoidRootPart.CFrame = dealerLoc.Torso.CFrame + Vector3.new(0, 0, 4)
        task.wait(1.5)
    end
    ...
    -- Find and click 'Take a look'
    local clickedInit = false
        ...
        if state then
            task.spawn(function()
                while getgenv().AutoLoot do
                    ...
                    if getgenv().ItemsLooted and getgenv().ItemsLooted >= threshold then
                        PerformAutoSell()
                        getgenv().ItemsLooted = 0
                        task.wait(1)
                    end

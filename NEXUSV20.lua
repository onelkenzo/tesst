...
getgenv().AutoInsurance = false
getgenv().InsuredItems = {} -- Track what needs to be insured

UILib:CreateToggle(MiscPanel, {
    Label = "Auto-Insurance on Death",
    Default = false,
    Callback = function(state)
        getgenv().AutoInsurance = state
        
        if state then
            -- Monitor character respawns
            task.spawn(function()
                LocalPlayer.CharacterAdded:Connect(function(newChar)
                    if not getgenv().AutoInsurance then return end
                    
                    -- Wait for character to fully load
                    newChar:WaitForChild("Humanoid")
                    newChar:WaitForChild("Cyberware")
                    task.wait(2) -- Extra delay for server to register
                    
                    UILib:CreateNotification({Text = "💀 Death detected! Processing insurance...", Duration = 3, Color = UILib.Colors.WARNING})
                    
                    local remotes = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes")
                    if not remotes then return end
                    
                    local replicateRemote = remotes:FindFirstChild("Replicate")
                    local cyberwareRemote = remotes:FindFirstChild("Cyberware")
                    
                    if not replicateRemote or not cyberwareRemote then return end
                    
                    -- STEP 1: Reclaim insurance
                    print("[AUTO-INSURANCE] Step 1: Reclaiming...")
                    replicateRemote:FireServer("ReclaimItemsProtectedByInsurance")
                    task.wait(1)
                    
                    -- STEP 2: Re-equip cyberware that was lost
                    print("[AUTO-INSURANCE] Step 2: Re-equipping...")
                    local cyberwareSlots = {"Arms", "Legs", "Sandevistan"}
                    
                    for _, slot in ipairs(cyberwareSlots) do
                        local slotValue = newChar.Cyberware:FindFirstChild(slot)
                        if slotValue and slotValue.Value == "None" then
                            -- This slot is empty, check backpack for items
                            for _, tool in ipairs(LocalPlayer.Backpack:GetChildren()) do
                                if tool:IsA("Tool") and tool:HasTag("Cyberware") then
                                    -- Try to install this cyberware
                                    print("[AUTO-INSURANCE] Installing:", tool.Name)
                                    cyberwareRemote:FireServer("Install", tool.Name)
                                    task.wait(0.5)
                                    
                                    -- Check if successful
                                    if slotValue.Value ~= "None" then
                                        break -- Slot filled, move to next
                                    end
                                end
                            end
                        end
                    ...
                    -- STEP 3: Re-purchase insurance for installed cyberware
                    print("[AUTO-INSURANCE] Step 3: Re-purchasing insurance...")
                    ...
                    for _, slot in ipairs(cyberwareSlots) do
                        ...
                        if slotValue and slotValue.Value ~= "None" then
                            ...
                            local baseCost = ItemsCost[itemName]
                            
                            if baseCost then
                                local insuranceCost = math.floor(baseCost * 0.7)
                                print(string.format("[AUTO-INSURANCE] Insuring %s for $%d", itemName, insuranceCost))
                                replicateRemote:FireServer("InsurancePurchase", itemName, 0.7, insuranceCost)
                                task.wait(0.5)
                            end
                        end
                    end
                    ...
                    print("[AUTO-INSURANCE] Complete!")
                end)
            end)
            
            UILib:CreateNotification({Text = "Auto-Insurance: ON\nWill handle everything on death!", Duration = 3, Color = UILib.Colors.SUCCESS})
        else
            UILib:CreateNotification({Text = "Auto-Insurance: OFF", Duration = 2})
        end
    end
})
...
-- Initialize callback storage for config auto-activation
...
UILib:CreateToggle(MiscPanel, {
    Label = "Enemy ESP",
    Default = Config.EnemyESP or false,
    Callback = function(state)
...
UILib:CreateToggle(MiscPanel, {
    ...
    Default = Config.RecoilBypass or false,
    Callback = function(state)
        local callback = function(s)
            getgenv().BypassRecoilSprint = s
            Config.RecoilBypass = s
            SaveConfig()
    ...
    ToggleCallbacks.RecoilBypass = callback
    callback(state)
    end
})
...
UILib:CreateToggle(MiscPanel, {
    Label = "Auto-Insurance on Death",
    Default = Config.AutoInsurance or false,
    Callback = function(state)
        local callback = function(s)
            getgenv().AutoInsuranceEnabled = s
            Config.AutoInsurance = s
            SaveConfig()
    ...
    ToggleCallbacks.AutoInsurance = callback
    callback(state)
    end
})
...
-- Store toggle callbacks for manual activation
local ToggleCallbacks = {}
...
task.spawn(function()
    ...
    if Config.EnemyESP and ToggleCallbacks.ESP then
        ToggleCallbacks.ESP(true)
        print("[Config] ✅ ESP activated")
    end
    
    if Config.RecoilBypass and ToggleCallbacks.RecoilBypass then
        ToggleCallbacks.RecoilBypass(true)
        print("[Config] ✅ Recoil Bypass activated")
    end
    
    if Config.AutoInsurance and ToggleCallbacks.AutoInsurance then
        ToggleCallbacks.AutoInsurance(true)

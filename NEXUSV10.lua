...
local function PerformAutoSell()
    ...
    -- CRITICAL FIX: Wait for dialogue to finish rendering (User reported ~6 seconds needed)
...
-- NPC and Quest Teleport Dropdown
...
-- Create dropdown with all locations
local locationNames = {}
local locationMap = {}
...
UILib:CreateDropdown(TeleportPanel, {
...
-- Cash Steal Farm Panel
local CashStealPanel = Window:CreatePanel({
    Name = "CashSteal",
    DisplayName = "Cash Steal Farm"
})
...
UILib:CreateSlider(CashStealPanel, {
    Text = "Dupe Count",
    Min = 10,
    Max = 500,
    Default = 100,
...
UILib:CreateToggle(CashStealPanel, {
    Label = "Dupe",
    Callback = function(state)
        getgenv().CashStealActive = state
        
        if state then
            UILib:CreateNotification({Text = "Cash Steal Farm Started!", Duration = 3})
            
            task.spawn(function()
                while getgenv().CashStealActive do
                    SafeExecute("CashStealLoop", function()
                        -- 1. Find nearby players in Workspace.Characters
                        ...
                        for _, character in ipairs(characters:GetChildren()) do
                            if character ~= LocalPlayer.Character and character:FindFirstChild("Humanoid") then
                                if character.Humanoid.Health > 0 then
                                    -- Try to hack them
                                    local success = PerformCashSteal(character)
                                    if success then
                                        UILib:CreateNotification({Text = "Hacked " .. character.Name .. "!", Duration = 1})
                                        targetFound = true
                                        ...
                                        if collected > 0 then
                                            UILib:CreateNotification({Text = "Collected " .. collected .. " money drops!", Duration = 2})
                                        end
                                        
                                        task.wait(3) -- Cooldown before next hack
                                        break
                                    end
                                end
                            end
                        ...
                        if not targetFound then
                            UILib:CreateNotification({Text = "No targets in range. Waiting...", Duration = 2})
                            task.wait(5)
                        end
                    end)
            ...
            UILib:CreateNotification({Text = "Cash Steal Farm Stopped", Duration = 2})
        end
    end
})

UILib:CreateButton(CashStealPanel, {
    Text = "Collect Dropped Money (Manual)",
    Callback = function()
        local collected = CollectDroppedMoney()
        if collected > 0 then
            UILib:CreateNotification({Text = "Collected " .. collected .. " drops!", Duration = 3})
        else
            UILib:CreateNotification({Text = "No money found!", Duration = 2, Color = UILib.Colors.WARNING})
        end
    end
})
...
-- Dismantle All Panel
local DismantlePanel = Window:CreatePanel({
    Name = "Dismantle",
    DisplayName = "Dismantle All"
})
...
UILib:CreateButton(DismantlePanel, {
